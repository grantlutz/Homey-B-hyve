...
class StateStore extends EventEmitter {
  ...
  setDevices(list) {
    ...
    for (const device of list || []) {
      ...
      if (previous) this._diffForTriggers(previous, device);
    }
  ...
  /**
   * The websocket can miss events; the reconciliation poll must still fire
   * the same semantic events when it detects the state changed.
   */
  _diffForTriggers(previous, device) {
    const before = previous.status || {};
    const after = device.status || {};

    const wasStation = before.watering_status?.current_station;
    const nowStation = after.watering_status?.current_station;
    if (wasStation !== nowStation) {
      if (wasStation !== undefined) {
        this.emit('watering_stopped', {
          device,
          station: wasStation,
          program: before.watering_status?.program,
          startedAt: before.watering_status?.started_watering_station_at,
        });
      }
      if (nowStation !== undefined) {
        this.emit('watering_started', {
          device,
          station: nowStation,
          program: after.watering_status?.program,
          runTimeMinutes: Number(after.watering_status?.run_time) || 0,
        });
      }
    }

    const wasDelay = Number(before.rain_delay) || 0;
    const nowDelay = Number(after.rain_delay) || 0;
    if ((wasDelay > 0) !== (nowDelay > 0)) {
      this.emit('rain_delay_changed', { device, hours: nowDelay });
    }

    if (before.run_mode !== after.run_mode && after.run_mode) {
      this.emit('mode_changed', { device, mode: after.run_mode });
    }

    const nowFaults = after.station_faults || [];
    if (nowFaults.length && !(before.station_faults || []).length) {
      this.emit('fault', { device, faults: nowFaults });
    }
  }
  ...
  applyEvent(event) {
    ...
    switch (event.event) {
      case 'watering_in_progress_notification': {
        ...
        const wasStation = previous?.current_station;
        // A program advancing to the next station means the previous zone
        // finished — surface that as a stop so its triggers/meters fire.
        if (previous && wasStation !== undefined && wasStation !== event.current_station) {
          this.emit('watering_stopped', {
            device,
            station: wasStation,
        ...
        if (event.rain_sensor_hold !== undefined) {
          device.status.rain_sensor_hold = event.rain_sensor_hold;
        }
        this._emitDevice(deviceId);
      ...
      case 'rain_delay': {
        ...
        // Cause/weather from the previous delay would be stale — use the
        // event's values or nothing.
        device.status.rain_delay_cause = event.rain_delay_cause || event.cause || '';
        device.status.rain_delay_weather_type = event.rain_delay_weather_type || event.weather_type || '';
        this._emitDevice(deviceId);
